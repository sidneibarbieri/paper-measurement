...
def restore_operations():
    ...
    if not ops_path.exists():
        print(f"❌ File not found: {OPERATIONS_FILE}")
        return
    ...
    for op in operations:
        try:
            ...
            if resp.status_code == 200:
                print(f"✅ Restored operation: {op.get('name')} ({op.get('id')})")
            else:
                print(f"⚠️ Failed to restore {op.get('name')}: {resp.status_code} - {resp.text}")
        except Exception as e:
            print(f"❌ Error restoring {op.get('name')}: {e}")
