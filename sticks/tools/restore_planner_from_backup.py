...
def restore_planners():
    with open(PLANNERS_FILE, "r", encoding="utf-8") as f:
        try:
            ...
            print(f"❌ Failed to parse {PLANNERS_FILE}: {e}")
            return
    ...
    for planner in planners:
        ...
        if resp.status_code == 200:
            print(f"✅ Restored planner: {planner.get('name', 'unknown')}")
        else:
            print(f"⚠️ Failed to restore planner {planner.get('name', 'unknown')}: {resp.status_code} {resp.text}")
